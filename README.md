...
### 访视计划 `/visit-plan`
- **方案与窗口期**：每个访视以「相对锚点天数」表达（相对随机化日 / 相对上一次实际访视日），
  窗口为计划日 ±N 天。研究统一口径为**「上一次实际访视日期优先，无实际记录时回退随机化方案日」**，
  口径在总览横幅、访视详情明确写出；两种口径推算不一致时给出醒目标记与备选日期，不让研究者猜。
- **方案修订冻结**：DM 可发布新版本（生效日）。发布后**已完成 / 已跳过 / 已锁库**访视冻结旧版原样不动，
  **尚未发生**的访视切换新版并按新偏移/窗口重算；新增访视补建、删除访视移除（仅未发生）。
  同一受试者新旧并存时，甘特图（虚线框 +「旧版冻结」标签 + 紫色徽标）与详情页均显眼提示。
- **跳过 / 计划外访视**：可跳过（计入留痕、不计完成度分母）、可恢复、可插入计划外访视；
  后续访视计划日按锚点规则**链式重算**。环依赖有 DFS 防御（结构上锚点只指更早访视），
  **已锁库访视绝不被重算/改期/跳过改动**。
- **视图与改期**：同时支持**月视图 / 周视图**；桌面甘特支持**拖拽改期**，落点超出窗口时
  弹出**二次确认 + 强制填写原因**，作为方案偏离（PD）写入排程留痕（含旧/新日期、操作人）。
- **三端呈现与状态**：>1180px **桌面甘特**（受试者为行、日期为列、窗期色带）、
  641–1180px **平板两栏日程**（左受试者右日程）、≤640px **手机竖向列表**。
  三种局面有各自文案：**加载失败**（错误态 + 重试）、**尚无任何访视**（引导去入组）、
  **访视被全部跳过**（引导恢复/核对状态），不使用统一的「暂无数据」。
- **完成度口径（平台自决并在界面明示）**：采用**「已完成并提交的关键表单（CRF）数 /
  关键表单总数」**。只填了部分字段、未完成提交的表单（半张表）**计 0**，不按字段数折算；
  已跳过访视不计入分母。这样「只填半张表的访视」在本口径下为 0%（字段口径约 50%），
  结论相反但符合 GCP「关键数据完整可核」的判定。**总览页 / 访视详情页 / CSV 导出三处
  数字由同一服务函数计算，保证一致**（导出按钮在右上角，含每个访视与受试者总完成度）。
- **时区**：所有日期按 **UTC 日历日存储**（取 12:00 UTC 规范时刻，DST 下也不跨午夜），
  按**研究中心所在 IANA 时区**展示与判定「今天/是否超窗」；跨中心 DM 视图统一北京时间。
  跨时区与夏令时「是否同一天」均经 zoneinfo 换算后判定。
- **锁库**：单个访视可锁库（计划/实际日期/表单全部冻结），解锁需原因留痕。
...
backend/   FastAPI + SQLAlchemy 2 + PostgreSQL + Redis + PyJWT
  ...
  app/scheduling.py        访视排程纯规则：UTC↔中心时区/DST、双锚点链式重算、
                           环检测、锁库保护、修订冻结切版、关键表单完成度（唯一事实源）
  app/schedule_service.py  ORM↔排程规则桥梁 + 统一序列化（总览/详情/导出三处一致）+ CSV
  app/routers/             auth / dashboard / subjects / sites / visit_plans
  app/seed.py              幂等种子（v1.0/v1.1 方案、跨版本并存、锁库/计划外/全跳过/半张表）
frontend/  Vue 3 + Vite + Element Plus + Pinia
  views/                   Login / Dashboard / SubjectList / SubjectDetail / VisitPlan
  components/visit/        甘特 / 平板两栏 / 手机竖列 / 改期弹窗 / 详情抽屉 / 修订发布
```
